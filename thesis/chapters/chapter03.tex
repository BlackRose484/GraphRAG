...
Một số ví dụ về cách gán nhãn sau khi mở rộng:

\begin{tcolorbox}[colback=gray!5, colframe=black!50, arc=4pt, boxrule=0.5pt]
{\small
\begin{verbatim}
Thị Kính:     :Character + :Main_Character        (loại Đào)
Súy Vân:      :Character + :Main_Character        (loại Đào)
Sùng Bà:      :Character + :Supporting_Character  (loại Mụ)
              + :Antagonist                       (vai phản diện)
Trần Phương:  :Character + :Supporting_Character + :Antagonist
\end{verbatim}
}
\end{tcolorbox}

Khác với năm vai truyền thống Đào/Kép/Hề/Lão/Mụ vốn xác định bởi đặc trưng diễn xướng, lớp \texttt{Antagonist} được định danh theo \textit{chức năng tự sự} trong vở. Việc gán nhãn này được thực hiện thủ công ở giai đoạn làm giàu đồ thị tri thức dựa trên trường \texttt{description} của ontology gốc, với tiêu chí: nhân vật có hành động chủ động gây ra xung đột chính của vở. Một nhân vật có thể đồng thời mang vai truyền thống và nhãn \texttt{Antagonist} mà không chồng chéo về ngữ nghĩa.

\textbf{Mở rộng phân loại biểu diễn.}
Lớp \texttt{Appearance} trong đồ thị cơ sở lưu trữ mọi lần xuất hiện của nhân vật trên sân khấu dưới dạng thực thể duy nhất, không phân biệt thể loại biểu đạt - một lần xuất hiện có thể đồng thời chứa cử chỉ tay, trang phục và lời hát, nhưng tất cả đều bị gộp vào cùng một loại nút. Với hệ thống hỏi-đáp có thể nhận câu hỏi về từng loại yếu tố biểu diễn riêng lẻ, lớp \texttt{Appearance} được phân thành ba nhóm con độc lập về ngữ nghĩa:
...
{\small
\begin{verbatim}
Câu hỏi: "Những cử chỉ tay nào xuất hiện trong trích đoạn
          Thị Màu lên chùa?"

Đồ thị cơ sở:    duyệt toàn bộ :Appearance trong trích đoạn
                 → lọc theo thuộc tính chuỗi mô tả
Đồ thị mở rộng:  truy xuất trực tiếp các nút :Hand_Gesture
                 trong trích đoạn
\end{verbatim}
}
\end{tcolorbox}

Đồ thị tri thức cuối cùng vì vậy bao quát đầy đủ tri thức của ontology gốc, đồng thời bổ sung tám phân lớp con phục vụ hai loại truy vấn đặc thù nêu ở đầu mục (Bảng~\ref{tab:schema_extension}, Hình~\ref{fig:kg_schema}).

\begin{table}[ht!]
  \centering
  \caption{Tám lớp con bổ sung trong đồ thị tri thức Chèo.}
...
Đồ thị mở rộng chứa toàn bộ tri thức tường minh của miền, nhưng nhiều câu hỏi thực tế đòi hỏi khai thác \textit{tri thức ngầm} - các quan hệ phải suy ra qua chuỗi liên kết nhiều bước. Tuỳ theo tần suất và chi phí, hệ thống áp dụng song song hai chiến lược: (i) \textit{vật chất hoá} trước các quan hệ tần suất cao thành cạnh tường minh để truy vấn hằng số, và (ii) \textit{duyệt động} qua đường đi nhiều bước cho các quan hệ còn lại. Cách kết hợp này cân bằng giữa chi phí lưu trữ và độ trễ truy vấn.

Phân tích tập câu hỏi thường gặp cho thấy mẫu ``diễn viên~$x$ tham gia vở nào?'' có tần suất cao nhất; trong đồ thị gốc, mỗi truy vấn loại này phải duyệt chuỗi bốn bước qua các nút trung gian:
...
Quan hệ \texttt{IS\_PERFORMED\_IN}~(Actor\,$\to$\,Play) được vật chất hoá bằng cách thực thi chuỗi~\eqref{eq:chain_actor_play} một lần tại thời điểm xây dựng đồ thị và lưu kết quả như cạnh tường minh. Đổi lại chi phí lưu trữ tối đa $O(|A| \cdot |P|)$ - tỉ lệ với tích số diễn viên và số vở chèo - mỗi truy vấn tần suất cao được đưa về chi phí hằng số $O(1)$.

Các quan hệ gián tiếp khác - như nhân vật tới các diễn viên đảm nhận, vở tới tập diễn viên đầy đủ kèm thông tin phiên bản hoặc vai diễn cụ thể, hay phép giao tập diễn viên giữa hai vở - không được vật chất hoá do phụ thuộc vào tham số đầu vào hoặc đòi hỏi thông tin trung gian. Các trường hợp này được xử lý bằng phép duyệt đường đi với độ sâu biến đổi đi qua các nút trung gian (RoleAssignment, Version, Scene), cho phép kho lưu trữ tự tối ưu theo cấu trúc thực tế của đồ thị mà không cần cố định số bước duyệt ở mã ứng dụng. Toàn bộ logic suy luận được điều khiển ở tầng ứng dụng, không phụ thuộc vào bộ suy luận ontology toàn cục, nhờ đó duy trì độ trễ thấp và kiểm soát được phạm vi suy luận theo từng loại câu hỏi.
...
\textbf{Bước 1 - Mở rộng và phân rã truy vấn.} Hai tác vụ LLM được thực hiện song song (Hình~\ref{fig:entity_extraction}) để giảm độ trễ xuống còn $\max(t_A, t_B)$ thay vì $t_A + t_B$:
...
\textbf{Ánh xạ mức truy xuất theo loại truy vấn.} Mỗi loại truy vấn chỉ kích hoạt tập con các mức phù hợp, thay vì chạy đồng loạt tất cả gây tốn tài nguyên (Bảng~\ref{tab:level_mapping}).
...
\textbf{Đầu ra.} Đối tượng dữ liệu đồ thị $G$ (Hình~\ref{fig:multi_granularity}) chứa kết quả từ các mức được kích hoạt:
